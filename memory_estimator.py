...
def memory_estimation(model_name: str, quantization_8bits: bool, quantization_4bits: bool, N_repeat: int = 10):
    ...
    cases the size of the cache is very small compared to the memory needed to compute it the first case, in which
    case the memory only scale with the memory footprint of the first forward pass.
...
if __name__ == '__main__':
    ...
    if num_gpus > 1:
        ...
        utils.dispatch_jobs(gpu_footprints, num_gpus, memory_estimation, models, int8, int4, N)
    else:
        for model in models:
